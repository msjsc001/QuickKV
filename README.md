...
*   **多行词条支持 (子内容)**: 支持将词条组织为父子结构，实现更复杂的模板和代码片段管理。
*   **智能搜索**:
    *   支持对父级词条进行关键词模糊搜索和多关键词筛选。
    *   **支持拼音首字母搜索**，输入 `qd` 即可匹配“请定”。
*   **自动排序**: 词库和搜索结果会根据父级词条的拼音首字母自动排序，保持内容清晰有序。
*   **快速粘贴**: 选中词条后，自动将完整内容（父级+子级，或仅子级）以“输入”的方式粘贴到光标位置。
...
*   **修复与优化**
    ...
    *   用户现在可以在系统托盘菜单中“重建快捷键钩子”子菜单下，选择“立即重建”或“设置自动重建间隔”（默认为5分钟，0表示禁用）。

### V1.0.4 (2025-07-21)
*   **新功能：子内容支持**
    *   现在支持多行词条。以 `- ` 开头的行被视为“父级”（用于搜索和排序），其下的所有行（包括空行和缩进行）都被视为“子内容”。
    *   选择一个词条后，会将父级及其所有子内容完整地粘贴出来。
    *   搜索和排序功能现在只针对父级行，但会保持整个内容块的完整性。
*   **新功能：父项不输出语法**
    *   在父级行末尾添加 ` ``不出现`` ` 语法后，粘贴时将只输出子内容，父级行本身会被忽略。这对于创建模板非常有用。
*   **UI 增强**
    *   搜索列表现在可以完整地展示多行词条，并以灰色字体区分父级和子级，使结构一目了然。
