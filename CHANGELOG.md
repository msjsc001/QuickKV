...
- ✔️**1.3.5 默认输入主链稳定化回退：恢复广泛可用性**
	- **回退：默认落字主链恢复为 `v1.3.4` 风格**。弹窗选词与快捷码再次统一收口到“复制到剪贴板 + 延迟执行 `perform_paste()`”的稳定主链，不再让实验性兼容层接管所有输出。
	- **回退：移除失败的统一输入兼容层**。删除 `smart_compat`、按软件记忆、窗口快照、焦点恢复、Win32 `SendInput` 默认注入，以及对应的 `input_compat` 运行时代码与 `pywin32` 依赖。
	- **保留：后续稳定功能不受影响**。别名搜索、文件夹词库、收藏排序、中文模板、剪贴板历史、热键与自动重启等既有能力继续沿用。
	- **恢复：快捷码回到“检测器 + 复用默认主链”**。命中后先删除触发码，再直接复用正常选词输出链，避免再让快捷码承担额外的兼容性实验逻辑。
	- **迁移：旧配置中的 `Paste.mode=smart_compat` 自动归一到 `ctrl_v`**。保存配置后不再写出已废弃的模式值。
	- **更新：README 与本地架构文档同步清理**。对外说明重新回到三种全局手动粘贴方式，避免文档与程序真实行为不一致。

- ✔️**1.3.4 语法兼容性优化：快捷码与别名更好写了**
	- **优化：快捷码写法更宽松**。现在 ` ``k:xxx`` `、` ``K:xxx`` `、` ``k：xxx`` `、` ``K：xxx`` ` 都能识别，不再卡大小写和中英文冒号。
	- **优化：别名写法更统一**。别名现在统一使用单个冒号写法，例如 ` ``bm:别名1、别名2`` ` 或 ` ``BM：别名1、别名2`` `，`bm/BM` 与 `:/：` 都支持。
	- **调整：旧的双冒号别名写法退出**。原来的 ` ``BM::别名1、别名2`` ` 不再作为正式写法，后续统一推荐使用单个冒号。
	- **更新：README 文档同步调整**。用户手册和功能说明已经一起更新到新的快捷码与别名写法。

- ✔️**1.3.3 功能增强：收藏排序与中文模板变量**
	- **新增：收藏置顶与最近使用微调**。现在可以在搜索结果中右键 `收藏/取消收藏` 普通词条。收藏和最近使用不会粗暴打乱原搜索逻辑，而是只在相关度相近的结果之间做温和排序增强。
	- **新增：收藏标识显示**。当开启“词条-归属显示”后，已收藏词条会在归属徽章前显示 `⭐`，方便一眼识别常用内容。
	- **新增：中文模板变量**。词条现在支持 `{{今天}}`、`{{现在}}`、`{{剪贴板}}`、`{{输入:项目名}}`、`{{输入:姓名|默认:张三}}` 这类中文模板语法，动态内容会在最终输出前渲染。
	- **新增：汇总输入表单**。当一条词条中存在多个 `{{输入:...}}` 字段时，程序会一次性弹出一个输入表单统一收集，不再逐个打断式询问。
	- **加固：模板错误处理**。未识别变量、过滤器写错、把 `格式化` 用在错误对象上时，程序会明确提示并中止本次输出，不会把半成品或原始模板直接粘贴出去。
	- **加固：状态持久化与清理**。收藏和最近使用记录独立保存到本地状态文件，不污染 md 词库；词条编辑或删除后，失效状态会在重载时自动清理。

- ✔️**1.3.2 功能闭环更新：别名搜索、文件夹词库、剪贴板隔离、热键与重启加固**
	- **新增：别名搜索**。支持在词条父级行尾使用 ` ``BM::别名1、别名2，别名3`` ` 语法，为同一条内容配置多个搜索入口。别名只参与搜索，不污染展示和粘贴输出，并兼容既有 ` ``不出现`` ` 与 ` ``k:xxx`` ` 语法。
	- **新增：文件夹词库**。系统托盘 `词库选择` 菜单现在同时支持 `添加MD词库文件夹` 和 `添加MD文件`。文件夹词库第一版会自动加载所选目录当前层级下的 `.md` 文件，并继续保持每条结果绑定真实源文件。
	- **优化：文件写入目标选择**。当词条来源于文件夹词库时，“添加到词库”只会列出真实可写的 `.md` 文件，不会把抽象文件夹误当作写入目标。
	- **修复：剪贴板历史隔离**。关闭剪贴板记忆后，剪贴板历史不再混入普通搜索池；空搜索仅在开启剪贴板记忆时显示历史记录。
	- **修复：多行剪贴板条目的自动清理**。定时清理现在按完整块内容而不是仅按父级标题处理，多行剪贴板记录可以稳定过期删除；编辑后的剪贴板条目会按新内容重新计时。
	- **修复：自定义热键静默失败**。热键输入框、设置对话框和底层注册逻辑已经统一白名单规则，`tab`、`backspace`、仅修饰键及未支持主键将被直接拦截，避免“界面能设、底层不认”的情况。
	- **修复：自动重启后的热键失效风险**。自动重启改为先释放当前监听与热键，再派发新进程；若新进程启动失败，则保留当前实例并恢复关键服务，同时给出明确提示。
	- **加固：PowerShell 粘贴派发结果检查**。粘贴和重启相关的 detached 进程启动现在会显式检查返回结果，减少静默失败带来的排障困难。

- ✔️**1.3.1 新功能：剪贴板文字记忆·定时清除 (Clipboard Auto-Clear)**
	- 在系统托盘"剪贴板文字记忆"子菜单下新增"启用定时清除"开关和"设置清除时间…"选项，默认关闭。
	- 开启后，剪贴板记录将在用户设定的分钟数（默认10分钟）后自动从历史中清除，减少隐私残留风险。
	- 采用纯内存计时方案（MVP），不修改也不污染原有的纯文本 Markdown 词库文件结构，保持数据透明度。
	- 重启软件后所有现存记录将以启动时间重新开始计时，实现优雅降级。

- ✔️**1.3.0 杀手级更新：无痕全局连续字符串盲打触发 (Continuous String Trigger)**
	- **全新交互引擎**：引入防冲突的后台全局键盘输入缓冲区（Queue Track Engine）。现在可以直接在任何编辑器或输入框处（包含游戏、前台全屏软件等），通过连续快速盲打特定英文字符串（如 `//` 或 `jj`）来瞬间呼出 QuickKV 搜索框。
	- **自动回退补偿 (Auto-Erase Compensation)**：当触发字符串命中的瞬间，QuickKV 会使用底层键盘钩子在 0.1 秒内模拟击打退格键 (`Backspace`) 吃掉您刚输入的触发符号，确保您的原输入框内不留垃圾字符残余。享受魔术般的无痕式操作体验。
	- **双轨配置选项板**：彻底重构了 `托盘界面 -> 设置快捷键` 的交互 UI。现在支持传统系统快捷键模式与全新的“连续字符串触发”双开或单开共存。并修复了因引入新控件导致的暗色主题（Dark Theme）下勾选框及边框说明文字颜色不亮的视觉 Bug。

- ✔️**1.2.3 重大优化：极致性能防卫战与 UI 深度打磨**
	- **性能防抖 (Debounce)**：为搜索框引入 200ms 防抖定时器，打字极速敲击时不再引起全库频繁搜寻，彻底消灭 CPU 尖刺。
	- **渲染缓存 (Render Caching)**：重构底层 `QTextDocument` 渲染引擎，引入 LRU 内存缓存（包含高亮特征码比对）。在加载千行巨长列表并疯狂滚动时，实现 100% 零卡顿的“贴图”级平滑。
	- **重构 Markdown 编辑器**：优化【编辑】弹窗，全新定制 `MarkdownTextEdit` 引擎。强制开启代码等宽字体对齐，强行将制表符锁定为 4 空格物理宽度，解决多级无序列表渲染过宽的问题。同时默认开启文字自动换行，拓展初始视窗尺寸。
	- **修复词库重复加载 Bug**：彻底抹平 Windows 系统盘符 (`D:\` vs `d:\`) 导致的大小写歧义，词库不再出现重复或虚假挂载。
	- **清洁启动**：移除了 PySide6(Qt6) 环境下废弃的 HighDPI 手动声明，实现真正 100% 干净的零错误终端启动日志。

- ✔️**1.2.2 新增：词条视觉归属与自适应换行**
	- 新增 **词条-自动换行** 功能（系统托盘右键菜单中操作，默认关闭）。开启后，长词条将根据窗口宽度自动动态换行而不再被截取。
	- 新增 **词条-归属显示** 功能（系统托盘右键菜单中操作，默认关闭）。开启后，将以半透明美观的徽章样式在词条右下角展示该词条所属的文件名。分别适配了日间/夜间模式。
	- 底层列表项渲染引擎从字符串强算重构为基于 `QTextDocument` 的富文本布局模型，以支持更优雅的排版和多部件绘制。

- ✔️1.2.1 修复：主界面边缘尺寸调整失效问题
	- 修复了无边框窗口模式下，当鼠标悬停于列表或输入框外部边缘时，无法唤出拖拽缩放光标（上/下/左/右）的问题。
	- 优化了底层事件拦截机制，确保任何方向的边缘都能顺滑拖拽。
